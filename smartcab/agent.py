...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        ## TO DO ##
        ###########
        Q_values = pd.Series(self.Q[state])
        max_q = Q_values.max()
    ...
    def createQ(self, state):
        ...
        in_table = state in self.Q
        # If it is not, create a new dictionary for that state
        #   Then, for each action available, set the initial Q-value to 0.0
        if not in_table:
            current_state_dict = dict()
            for action in self.valid_actions:
                current_state_dict[action] = 0.0
            self.Q[state] = current_state_dict
    ...
    def choose_action(self, state):
        ...
        if not self.learning or  random.random() < self.epsilon: 
            ...
            action_values = pd.Series(self.Q[state])
            maxQ = self.get_maxQ(self.state)
            max_actions = action_values[ action_values == action_values.max() ].index
